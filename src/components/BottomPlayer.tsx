...
export default function BottomPlayer({
  ...
  return (
    <div 
      ...
      className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 w-[95%] md:w-[85%] max-w-4xl rounded-2xl md:rounded-full bg-[#1c1c1e] border border-white/10 px-4 md:px-6 py-3 md:py-3.5 shadow-[0_15px_50px_rgba(0,0,0,0.8)] flex items-center select-none animate-fade-in"
    >
      
      {/* DESKTOP LAYOUT (md:flex) */}
      <div className="hidden md:flex items-center justify-between w-full gap-4 md:gap-6">
      ...
      <div className="flex md:hidden items-center justify-between w-full gap-2.5">
        {/* Left: Album cover & title/artist (Click opens full immersive player) */}
        <div 
          onClick={onOpenFullScreen}
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <span className="block text-[10px] text-stone-400 truncate leading-tight mt-0.5">
              {currentTrack.artist}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            onClick={onPlayPause}
            className="p-2 bg-white text-stone-950 rounded-full hover:scale-105 active:scale-95 transition-all shadow-md"
            title={isPlaying ? 'Pausar' : 'Tocar'}
          >
            {isPlaying ? (
          ...
          <button
            onClick={onOpenFullScreen}
            className="p-1 text-[#dfb26f] hover:text-white transition-colors text-[10px] font-bold font-mono bg-[#dfb26f]/10 border border-[#dfb26f]/20 rounded-md ml-1.5 px-2 py-1 shrink-0"
            title="Ver letra e mais"
          >
            LETRA
          </button>
        </div>
